...
Short description
This repository runs the Home Energy Monitoring application using Docker Compose. This README references the actual
configuration and deployment files present in this repo.
...
- Common CI steps your project uses or should use:
    ...
    - On tag: build & push multi-arch images to GHCR (image name in compose:
      `ghcr.io/ksw1984/home-energy-monitoring:${HOME_ENERGY_VERSION}`)

### Integration Tests

1. Build a docker container image (takes around 10 min)
    ```
    docker buildx build `
...
2. Check that Docker built ARM64 (expected output `linux/arm64`)
    ```
    docker image inspect home-energy-monitoring:arm64 --format '{{.Os}}/{{.Architecture}}'
    ```

3. Export image
    ```
    docker save home-energy-monitoring:arm64 -o home-energy-monitoring-arm64.tar
    ```

4. Copy that .tar to the Pi
    ```
    /opt/home-energy-monitoring/home-energy-monitoring-arm64.tar
    ```

5. Load the image
    ```
    docker load -i /opt/home-energy-monitoring/home-energy-monitoring-arm64.tar
    ```

6. Create docker-compose.dev.yml to override
    ```
    services:
      home-energy-monitoring:
...
7. Remove the old dev container
    ```
    docker rm home-energy-monitoring-dev
    ```

8. Create the new dev container with override
    ```
    docker compose \
      -f compose.yml \
...
9. Restart the dev image on the Pi
    ```
    docker start -a home-energy-monitoring-dev
    ```
...
- Main app config: `config.yaml` (repo root) — used by the application. Example keys (already in your file):
  `collection.interval`, `collectors`, `databases`.
- Docker Compose environment:
    ...
    - Note: `deployment/compose.yml` itself contains an `env_file: ../.env` mapping for services (so the runtime expects
      `.env` at repo root). If `.env` is not tracked, create it from your secrets.
...
- If `.env` is missing, create `.env` in repo root. Compose will use `--env-file .env` and the file referenced inside
  `deployment/compose.yml` also expects `../.env`.
...
- If `.env` missing, create from your environment variables; `deployment/compose.yml` expects env values like
  `HOME_ENERGY_VERSION`, `INFLUXDB_VERSION`, `GRAFANA_VERSION`.
